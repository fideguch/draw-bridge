...
**Independent Test**: Can be fully tested by clearing any level and verifying the 5 beats play in order within a 3–4 second total, ending with Next activation in 1.5–2.5s.
...
2. **Given** the celebration completes, **When** waiting, **Then** the Next button activates 1.5–2.5s after completion, pulsing at scale ±5% with a 0.8s period, with Replay alongside.
...
- **TempoContract**: The numeric loop-speed contract — L1 clear ≤ 25s, first 3 levels 60–90s, fail→retry ≤ 1s, full loop ≤ 40s, Next activation 1.5–2.5s — enforced by automated ghost-replay and UI-transition tests.
...
- **Non-functional targets adopted as binding constraints (designs/non_functional_requirements.md)**:
  ...
  - NFR-003 tempo contract: L1 ≤ 25s / first 3 levels 60–90s / retry ≤ 1s / loop ≤ 40s / first full loop ≤ 45s / Next 1.5–2.5s (Must)
