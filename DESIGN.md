...
**危険シグナル（NEW — 予約パレット。HAZARD専用、他用途禁止）**

危険要素（岩・棘・危険帯）だけが使う予約色。UI・地形・コイン・橋・車には一切流用しない（原則5「色は意味に固定」の危険版）。淡い空（#A8E4FF）と茶の地形（#A06A3F）の**両方**で「危険」が一目で読めるよう、高彩度の赤/橙ファミリを近黒チャコールの土台＋輪郭に載せる（ハイパーカジュアルの危険配色定石: 高コントラスト・冗長符号化・サムネ可読）。部品規格は §4.9。

| token | HEX | 用途 |
| --- | --- | --- |
| `hazardRed` | #E11D0E | 主・危険赤（棘の先端、危険帯の塗り＋ハッチ、岩の亀裂） |
| `hazardRedDeep` | #8A0F06 | 深赤の影（棘の根、ハッチ縞、亀裂の芯） |
| `hazardBorderRed` | #B00D04 | 危険帯の濃赤ボーダー（帯を縁取り、どの背景でも読める） |
| `hazardWarn` | #FF9500 | 琥珀の予告（起爆前の岩オーラ・落下ビーム・標的リング） |
| `hazardWarnCore` | #FFF0C2 | 予告の明るい芯（「!」グリフ・ハイライト） |
| `hazardDark` | #161011 | 近黒チャコール（岩の縁/輪郭、棘の土台、全エッジ） |
| `hazardCharcoal` | #2A2327 | 温チャコール（岩の本体＝重い暗石） |
| `hazardStone` | #4A3F47 | やや明チャコールのハイライト（岩の立体感） |
| `hazardCrack` | #FF5A1E | 赤橙の亀裂/熱亀裂（岩・モーションストリーク） |

**危険色の隔離ルール（原則5の危険版）**: 上記9トークンは**危険要素以外に使用禁止**。`uiDanger`(#FF3B30, 破壊操作) / `stressHigh`(#FF3B30, 橋の応力最大) とは意味・色を分離する（前者はUI/橋の一過性、後者は地上の常設ハザード。危険色はより深い #E11D0E 系）。色覚多様性でも識別できるよう、色＋形＋動きで冗長符号化する（§4.9）。
...
### 4.9 危険要素（Hazard）— 予約シグナル体系（`src/render/world/`）

> Discord フィードバック「障害物の視認性が終わってる」への回答。ハザードは §3.1「危険シグナル」パレット＋固有シルエット＋動きで**冗長に**符号化し、サムネイルサイズでも空・地形の両背景で「危険」と即読できることを規格とする。全て fill-only（Phaser4 stroke バグ回避、research §3）。render 定数は `TuningConstants.hazardRender`、色は `theme.color` の `hazard*`。物理には一切触れない（`DangerZone.style` は render 専用の不活性タグ＝決定論ハッシュ不変）。

**共通原則**

- 予約パレット厳守（UI/地形と色を共有しない）。
- 近黒チャコール土台＋赤/橙で「高コントラスト」を空・地形の両方で確保。
- 色＋形＋動きの三重符号化（色覚多様性・低輝度でも識別）。
- サムネ可読（拡大せずとも危険と分かる）。

**4.9.1 Rock（落石／転石） — `RockRenderer.ts`**

| 項目 | 規格 |
| --- | --- |
| アナトミー | 近黒リム(`hazardDark`)＋温チャコール本体(`hazardCharcoal`)の重い暗石。中心から放射する赤橙の亀裂 facet（`hazardRedDeep` 下地＋`hazardCrack`）＋発光コア。左上に `hazardStone` ハイライトで立体感 |
| 回転 | 実 body 角で亀裂リングを回す（奇数 `FACET_COUNT`=7＝90°反復なし）→転がりが読める |
| 動き | 画面速度が `streakSpeedMinPx` 超で後方へ赤橙モーションストリーク（速度比で伸長・濃度、`streakLengthMult`/`streakAlphaMax`） |
| 状態 | `live`（実 body）＝上記の岩 / `armed`（起爆前・body 無し）＝下記の予告 |

**4.9.2 Armed 予告（触発落石の公平性キュー） — `RockRenderer.drawWarning`**

- アナトミー: 起爆前の岩スポーンに、脈動する琥珀オーラ(`hazardWarn`)＋太い「!」(`hazardWarnCore`)＋**落下ビーム**（下方向の半透明琥珀柱）＋着弾**標的レティクル**（`warnBeamLengthM` 下のリング＋赤クロス `hazardRed`）。
- 意図: 「ここに・下へ落ちて・ここに当たる」を明示（欠陥4 公平性）。起爆で同スロットが `live` の岩へシームレス遷移。

**4.9.3 DangerZone（危険帯／棘） — `DangerZoneRenderer.ts`**

| レイヤ | 規格 |
| --- | --- |
| 塗り | 高彩度赤ウォッシュ `hazardRed`（α `zoneFillAlphaMin..Max` で呼吸） |
| ハッチ | 45°深赤 `hazardRedDeep` 対角縞が**スクロール**（`zoneStripeScrollPxPerSec` の理容店ポール的クロール＝「生きた危険」） |
| ボーダー | 濃赤 `hazardBorderRed` で帯を縁取り |
| 形（style） | `spike`＝上向き鋸歯（床棘）/ `spikeDown`＝下向き鍾乳（天井）/ `zone`＝歯なしハッチ帯 |
| 歯の階調 | near-black 土台(`hazardDark`)→deep-red(`hazardRedDeep`)→red-tip(`hazardRed`)（`toothHeightFrac`/`toothTipFrac`/`teethPerMeter`） |
| 物理 | エンジンは常に base rect のみで衝突。`style` は描画専用（不活性） |

**4.9.4 スキーマ（additive, schemaVersion 1 不変）**

- `DangerZone.style?: 'zone' | 'spike' | 'spikeDown'`（語彙 `LevelSchema.DANGER_STYLES`）。欠落＝`zone`（後方互換・バイト同一）。`parseDangerZone` が語彙を検証、`goalFlag` 等の `Rect` は厳格なまま（`style` を許容しない）。

**4.9.5 アトラス整合（`scripts/atlas/levelCard.ts`）**

- ルートアトラスのカードも**同一パレット**で歯・岩・危険帯を描く（in-game と 1:1）。静的 SVG なのでスクロール/脈動は無く、色・形のみ一致させる。

**避けるべき（Avoid）**

- ✗ 岩をスレートグレー無地（低コントラスト）／✗ 棘を地形茶で描く（地形と混同）／✗ 危険帯を淡い赤 α<0.2（読めない）／✗ 危険色を UI・地形へ流用／✗ 色のみの符号化（形・動きを省く）。
...
| §3.1 危険シグナル予約パレット `hazard*` | `src/render/ui/theme.ts` `color` | MODIFY（キー追加、旧 rock*/hazard* を刷新） |
| §4.9 Rock 再配色＋亀裂＋ストリーク＋Armed予告 | `src/render/world/RockRenderer.ts` | MODIFY（大） |
| §4.9 DangerZone 赤ウォッシュ＋縞スクロール＋歯 | `src/render/world/DangerZoneRenderer.ts` | MODIFY（大） |
| §4.9.4 `DangerZone.style` additive タグ＋検証 | `src/engine/level/LevelSchema.ts`（`DangerStyle`/`DANGER_STYLES`/`parseDangerZone`） | MODIFY（追加、物理不活性） |
| §4.9 hazard render 定数 | `src/tuning/TuningConstants.ts` `hazardRender` | MODIFY（追加） |
| §4.9.5 アトラス歯/岩/危険帯を同一パレットに | `scripts/atlas/levelCard.ts` | MODIFY |
| §4.9 棘の危険帯を `style:'spike'` タグ付け | `scripts/levels/ch1.ts` + `levels/ch1-{l06,l11,l12,l15,b2}.json` | MODIFY（追加タグのみ） |
...
- [ ] 危険要素（岩/棘/危険帯）が §3.1 予約パレット＋固有シルエット＋動きで、空・地形の両背景でサムネ可読（§4.9）
- [ ] 危険色（`hazard*`）が UI/地形/橋/車に流用されていない（原則5の危険版）
- [ ] `DangerZone.style` が物理不活性（spike:determinism ハッシュ不変、gates/campaign 18/18 維持）
```
